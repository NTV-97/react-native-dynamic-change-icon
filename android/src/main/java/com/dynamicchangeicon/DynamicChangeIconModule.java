...
@ReactModule(name = DynamicChangeIconModule.NAME)
public class DynamicChangeIconModule extends ReactContextBaseJavaModule implements Application.ActivityLifecycleCallbacks {
  public static final String NAME = "DynamicChangeIcon";
    private final String packageName;
    private List<String> classesToKill = new ArrayList<>();
    private Boolean iconChanged = false;
    private String componentClass = "";

    public DynamicChangeIconModule(ReactApplicationContext reactContext, String packageName) {
        super(reactContext);
    ...
    public void getIcon(Promise promise){
        ...
        String[] componentClassSplit = this.componentClass.split("\\.");
        String currentIcon = componentClassSplit[componentClassSplit.length - 1];
    ...
    public void changeIcon(String enableIcon, Promise promise) {
        ...
        String[] componentClassSplit = this.componentClass.split("\\.");
        String activeClass = "";
        for (int i = 0; i < componentClassSplit.length - 1; i++) {
            if(activeClass.length() > 0) {
                activeClass += "." + componentClassSplit[i];
            } else {
                activeClass = componentClassSplit[i];
            }
        }
        activeClass += "." + enableIcon;
